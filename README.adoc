...
Follow the types. Have a look at what type the backend call needs and then figure out the UI bits that you need to get that together. Because the code in api/Generated/Api.elm is auto generated, it's sadly lacking in documentation. Check out the <<Backend API>> for API details as you need them.
...
* You can also peek at the link:./src/Answers/[Answers] if you get stuck.
====
...
* It's worth saving your file after each dot point to see what error messages that you get when you forget something. :)
====
...
- Hook up `HA.value` to the `model.loginPlayerId` and `model.loginPassword` so that the value properly flows back down from our model to the form element. If you forget to do this, if you ever update the value in your model it will not get updated in the form. 
...
- Your page should be compiling now. Open up the elm debugger by clicking "Explore History" on the lower right. If you type into the two inputs, you should see your events and new state in action!

[NOTE]
====
At this point you should be feeling fairly comfortable with:

* Making a basic form and saving the state of that input into the model. 
* Getting the shape of how your dom events flow in via your `Msg` type and the model gets updated in the `update` function. This is the core of "The Elm Architecture" so it's important to be pretty clear on this.

If you press enter or hit the button it just reloads the page for now. We'll fix this in the next step.
====
...
[TIP]
====
* We are now interacting with the backend api code generated from the haskell backend. You can see docs for these calls at <<Backend API>>.
* This is our first interaction with the https://guide.elm-lang.org/error_handling/result.html[Result], which is a way to handle errors in elm without exceptions. If you are unfamiliar with `Result` or (in haskell) `Either` then it is a good idea to read that part of the elm guide. 
====

First up, our backend call for logging in is:
...
We want to make that call when the user submits the form. So lets hook into the form's `HE.onSubmit` to make this happen

* There is a dummy call doing a `BE.postApilogin` when the page initialises in `init`. Lets remove it by changing the initialiser command to `Cmd.none`. Take note of the shape of this call though because we will use it shortly!
* Add a `LoginSubmit` to the `Msg` type. This constructor will take no arguments.
* Lets setup the work that we plan to do when the form is submitted:
** On the `update` function, add the `LoginSubmit` handler which fires off to the backend by calling `BE.postApiLogin` with the username and password from our model.
** Lets break down the shape of the backend call: `postApiPlayers : DbPlayer -> (Result Http.Error String -> msg) -> Cmd msg`
*** Takes a `dbPlayer` (a record alias with `dbUsername` and `dbPassword` fields)
*** Takes a "callback" function that takes the Http Response and turns it into a `Msg`.
*** Returns a cmd that can fit into the second spot in the tuple returned by update.
** If we aren't updating the model, you can't do a record update that updates nothing, so write it as ```(model, <call to postApiLogin goes here>)```. 
* Hook `HE.onSubmit` on the form element to the `LoginSubmit` Msg.
* If you try to login with the browser network tab open, you should see the call happen when pressing enter or the button. But there is zero feedback or much done with the response at this point. You should also see the response event in the elm debugger.
* Lets add some feedback in the simplest way that we know how:
** Remove `backendError` and `backendOk`
** Add some things to the model: `loginToken : Maybe String`, `loginErrorMessage: Maybe String`. Initialise them all to nothing.
** On the `HandleLoginResp` handler, set the `loginToken`, `loginErrorMessage` as appropriate.
** Display the token, and error message in the view via pattern matching the maybe. There's a handy class called `err` if you want to make the error look pretty and error-y. :)
** You probably will want to update the `LoginSubmit` handler to clear out the token and error message to `Nothing` when a new call to the backend is fired off.

This wont do much, but it will print an error if the user doesn't authenticate and show a obtuse string of characters if it worked. That's OK for now. We'll do a lot more with the token, going to other pages and cleaning up our handling of the response later. For now it feels a bit unfinished but just bear with us till the register page is done.

`playerId` / `password` that are already in the database ready for testing are user1 .. user5 with the password "pass".

[NOTE]
====
At this point you should now see:
* how we can fire off effects in our update and how the return from our backend call comes in a later msg like `HandleLoginResp`.
* Have an appreciation for `Result` and `Maybe` if you hadn't used them before. 
====
...
We'll just put the register underneath the login for now all on the one page (you'll have to wrap things in another div at the top level. Just reuse the login-box class for the register form).

Add new model fields for:
...
But with this form we have to do some client side validation of the form before we send off the backend call. On our submit handler (the thing that pulls apart `RegisterSubmit`, we want to check whether the passwords match and only call the register call if they match. 

A good place to start is to write a function with this signature:

```elm
-- Most validation functions probably want to return many errors so that it can give 
-- the most feedback to the user.
validateDbPlayer : Model -> Result.Result (List String) BE.DbPlayer
validateDbPlayer model =
```

So it takes the model and returns either a list of errors or a valid `DbPlayer` ready to submit to the backend. Try writing this with a simple if statement and checking whether `model.registerPassword == model.registerPasswordAgain`. We could also add validation that the password is not empty and that the username is a minimum of three characters, if you'd like.
...
Finish making sure the new view is all wired up and does what you'd expect (prints the validation errors, backend errors or the successful token). 

There's something new here that's worth mentioning! Here we have a list of validation issues and we need to print that into the dom somehow. If we look at the type of `H.ul`:

```elm
ul : List (H.Attribute msg) -> List (H.Html msg) -> H.Html msg
```

This means that if use List.map to create the list of children from our list of errors:

```elm
 H.ul [ HA.class "err" ] <| List.map (\err -> H.li [] [ H.text err ]) model.registerValidationIssues
```

If you've done any react work before, you may think that adding a list this way may be very unperformant. In this app it's fine to do things this way, but if you are after tips on how to make your views more performant, check out the https://guide.elm-lang.org/optimization/[Optimization] part of the Elm guide.

[NOTE]
====
At this point you should now see:

* Be more comfortable starting of a UI from scratch and wiring it up
* Get an idea of a simple form validation step.
* Know the most basic way to write out a list of things in your view.
* Be starting to get the feeling that there must be a cleaner way to lay things out! :)
====

## Exercise 3: (Refactor 1) Remote Data Pattern
...
This is the same shape as our two maybes, but we now also get feedback when the backend call is in progress, which is very handy! And we also make it so that the types properly represent that we cannot have an error and a token at the same time.
...
This section is to help you figure out the shape of the backend calls that can be found in link:./api/Generated/Api.elm[Generated.Api]. It is designed to be read alongside the elm code to give additional context that the autogenerated code is lacking. While the elm code is automatically generated, this documentation here is manually created and probably wrong. Always trust the elm types over the documentation if there is a disagreement.

The haskell code that describes the api can be found link:./dissidence/backend/src/Game/Dissidence.hs[here]. This api is written with servant, which if you are interested about you can read about at their website: https://www.servant.dev/. You do not need to understand this haskell code to complete this workshop, so don't worry if you find it confusing or don't care. That's perfectly OK. :)
