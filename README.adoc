...
** Note the type of this backend call: `postApiPlayers : DbPlayer -> (Result Http.Error String -> msg) -> Cmd msg`
...
** Add some things to the model: `token : Maybe String`, `loginErrorMessage: Maybe String`. Initialise them all to nothing.
** On the `HandleLoginResp` handler, set the `token`, `loginErrorMessage` as appropriate.
