...
- Write some new view that displays the backendError if the Maybe has a value (Pattern matching is fine or you can use `Utils.maybe`).
...
We'll just put the register underneath the login for now all on the one page (you'll have to wrap things in another div at the top level). Add new model fields and message constructors for `registerPlayerId`, `registerPassword`, `registerPasswordAgain`, `registerValidationIssues`, `SetRegisterPlayerId`, `SetRegisterPassword`, `SetRegisterPasswordAgain`.
...
```elm
        Submit ->
            case validateDbPlayer model of
                Ok dbPlayer ->
                    ( { model | registerValidationIssues = [] }
